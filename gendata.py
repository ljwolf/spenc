...
if __name__ == "__main__":
    ...
    np.random.seed(SEED)
    labels = spenc.SPENC(n_clusters=10).fit(None, natR.sparse).labels_
    labels.dump(os.path.join(datadir, 'nat_10k_nodata.ary'))

    print('(2 of 5) doing 30k sampling')
    np.random.seed(SEED)
    labels = spenc.SPENC(n_clusters=30).sample(natR.sparse, n_samples=3)
    labels.dump(os.path.join(datadir, 'nat_30k_randoms.ary'))

    print('(3 of 5) doing 30k withdata')
    np.random.seed(SEED)
    labels = spenc.SPENC(n_clusters=30, gamma = .001).fit(X, natR.sparse).labels_
    labels.dump(os.path.join(datadir, 'nat_30k_discovered.ary'))

    print('(4 of 5) doing infk sampling')
    np.random.seed(SEED)
    labels = spenc.SPENC(n_clusters=np.inf).sample(natR.sparse, floor=20)
    labels.dump(os.path.join(datadir, 'nat_infk_randoms.ary'))

    print('(5 of 5) doing infk withdata')
    np.random.seed(SEED)
    labels = spenc.SPENC(n_clusters=np.inf, gamma=.001).fit(X, natR.sparse, floor=20).labels_
